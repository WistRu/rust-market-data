...
use aster::{
    AsterFuturesCoverageConfig, AsterPublicRestClient, AsterSpotCoverageConfig, AsterSymbolInfo,
    OneOrMany, build_futures_public_subscriptions, build_spot_public_subscriptions,
    covered_symbols,
};
...
async fn main() -> Result<()> {
    ...
    let spot_plan =
        build_spot_public_subscriptions(&spot_symbols, &AsterSpotCoverageConfig::exhaustive());
    let futures_plan = build_futures_public_subscriptions(
        &futures_symbols,
        &AsterFuturesCoverageConfig::exhaustive(),
    );
    ...
    let futures_book_effective_symbols = recover_futures_book_ticker_symbols(
        &rest,
        &futures_trading_symbols,
        futures_book_symbols.clone(),
    )
    .await?;
    print_rest_coverage(
        "futures_book_ticker_effective",
        &futures_trading_symbols,
        &futures_book_effective_symbols,
    )?;
    print_rest_coverage(
        "futures_24hr_ticker",
        &futures_trading_symbols,
...
fn sorted_symbols(symbols: &[AsterSymbolInfo]) -> Vec<String> {
...
fn sorted_trading_symbols(symbols: &[AsterSymbolInfo]) -> Vec<String> {
    let mut result = symbols
        .iter()
...
fn status_breakdown(symbols: &[AsterSymbolInfo]) -> BTreeMap<String, usize> {
...
fn print_plan_coverage(
    label: &str,
    universe: &[String],
    subscriptions: &[aster::AsterWsSubscription],
) -> Result<()> {
    ...
    println!(
        "{label} subscriptions={} symbols_covered={}/{} coverage_pct={pct:.2} missing={:?}",
        subscriptions.len(),
...
fn print_rest_coverage(
    ...
    println!(
        "{label} trading_symbols_seen={}/{} coverage_pct={pct:.2} missing={:?}",
        required_symbols.len() - missing.len(),
        required_symbols.len(),
        missing
    );
...
async fn recover_futures_book_ticker_symbols(
    rest: &AsterPublicRestClient,
    required_symbols: &[String],
    mut seen_symbols: BTreeSet<String>,
) -> Result<BTreeSet<String>> {
    for symbol in missing_symbols(required_symbols, &seen_symbols) {
        if symbol_from_one_or_many(rest.futures_book_ticker(Some(&symbol)).await?).as_deref()
            == Some(symbol.as_str())
        {
...
fn symbol_from_one_or_many(value: OneOrMany<Value>) -> Option<String> {
    match value {
        OneOrMany::One(value) => symbol_from_value(&value),
        OneOrMany::Many(mut values) => values.pop().and_then(|value| symbol_from_value(&value)),
    }
}

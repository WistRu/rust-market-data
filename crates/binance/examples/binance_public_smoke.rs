use anyhow::Result;
use binance::{BinancePublicRestClient, BinanceWsClient, BinanceWsSubscription};
...
async fn main() -> Result<()> {
    ...
    let mut stream = BinanceWsClient::futures()
        .connect_streams(vec![BinanceWsSubscription::BookTicker {
            symbol: symbol.clone(),
        }])
        .await?;
